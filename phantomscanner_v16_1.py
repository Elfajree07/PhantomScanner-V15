...
def classify_proof_status(kind, evidence, *, observed=False, canary_reflected=False,
                          location_verified=False, cors_verified=False):
    """
    Proof-oriented classification for safe, non-destructive validation.

    This deliberately distinguishes an observed security condition from a
    demonstrated exploit. It never executes destructive payloads.
    """
    ev = (evidence or "").lower()

    if kind == "header" and observed:
        return "CONFIRMED"
    if kind == "cookie" and observed:
        return "CONFIRMED"
    if kind == "redirect" and location_verified:
        return "CONFIRMED"
    if kind == "cors" and cors_verified:
        return "CONFIRMED"
    if kind == "reflection" and canary_reflected:
        return "CONFIRMED_REFLECTION"
    if kind == "error_disclosure" and observed:
        return "CONFIRMED_ERROR_DISCLOSURE"

    # Explicitly avoid calling an exploit proven from a weak signature.
    if any(x in ev for x in ("possible", "suspected", "potential", "signature")):
        return "POTENTIAL"
    return "INFO"
